...
Evaluator.prototype.visitMember = function(node){
  ...
  if ('object' != obj.nodeName && 'atblock' != obj.nodeName) {
...
Evaluator.prototype.visitBlock = function(block){
  ...
  for (block.index = 0; block.index < block.nodes.length; ++block.index) {
    try {
      var node = block.nodes[block.index]
      
      // when there is a property
      // that has an object as value
      // treat the keys as screen size names
      // and call the screen-size mixing for each key
      // with the according value
      /*if (node.nodeName === 'property') {
        var isSizeResponsive = false;
        node.expr.nodes.forEach(node => {
          if (node.nodeName === 'object') {
            isSizeResponsive = true;
          }
        })
      
        if (isSizeResponsive) {
          var wrapperGroup = new nodes.Group()

          var wrapperSelector = new nodes.Selector([new nodes.Literal('&')])
          wrapperGroup.nodes.push(wrapperSelector)

          var wrapperBlock = new nodes.Block(block, wrapperGroup)
          wrapperGroup.block = wrapperBlock

          var responsiveValues = node.expr.nodes[0].vals

          Object.keys(responsiveValues).forEach(function(key) {
            var responsiveValue = responsiveValues[key]

            var sizeArgument = new nodes.Expression(false)
            sizeArgument.nodes.push(new nodes.Ident(key));

            var callArguments = new nodes.Arguments();
            callArguments.nodes.push(sizeArgument);

            var responsiveCall = new nodes.Call('screen-size', callArguments);
  
            var responsiveBlock = new nodes.Block(wrapperBlock, responsiveCall);
            var property = new nodes.Property(node.segments, responsiveValue);
            property.name = node.name;
            responsiveBlock.nodes.push(property);

            responsiveCall.block = responsiveBlock;

            wrapperBlock.nodes.push(responsiveCall)
          })
      ...
      block.nodes[block.index] = this.visit(node);
    } catch (err) {
  ...
  for (var i = 0; i < len; ++i) {
    switch ((node = items[i]).nodeName) {
      ...
      case 'expression':
        ...
        break;
      case 'object':
        // taken from this.mixinObject(object) function and modified a little to work here
        var object = node
        var Parser = require('../parser')
          , root = this.root
          , str = '$block ' + object.toBlock()
          , parser = new Parser(str, utils.merge({ root: null }, this.options))
          , tmpBlock;
      
        try {
          tmpBlock = parser.parse();
        } catch (err) {
        ...
        tmpBlock.parent = root;
        tmpBlock.scope = false;
        var ret = this.visit(tmpBlock)
          , vals = ret.first.nodes;
        for (var i = 0, len = vals.length; i < len; ++i) {
          if (vals[i].block) {
            //this.mixin(vals[i].block.nodes, this.currentBlock);
            this._mixin(vals[i].block.nodes, dest, block)
            break;
          }
        }
        break;
